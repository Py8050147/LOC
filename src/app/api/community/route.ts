...
export async function POST(request: Request) {
    ...
    try {
      ...
      courseId: data.get("courseId"),
        ...
        return  Response.json({ message: error.message || 'Validation failed' }, { status: 400 });
    }
    ...
    try {
        ...
        return  Response.json({ message: 'Failed to save the file to the filesystem' }, { status: 500 });
    }
    ...
    try {
        ...
        return  Response.json({ message: 'Failed to store community into the database' }, { status: 500 });
    }
...
export async function GET() {
    try {
        const allCommunity = await db.select({
            id: community.id,
            user: users.fname,
